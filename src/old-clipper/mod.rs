...
impl Default for Clipper<'_, Unknown, Unknown, Unknown, Unknown> {
    fn default() -> Self {
        Self {
...
impl<'a, Sub, Clip, Op, Tol> Clipper<'a, Sub, Clip, Op, Tol> {
    pub(crate) fn with_operator<Operator>(self) -> Clipper<'a, Sub, Clip, Operator, Tol> {
        Clipper {
            operator: PhantomData,
...
impl<'a, Sub, Op, Tol> Clipper<'a, Sub, Unknown, Op, Tol> {
    pub(crate) fn with_clip<U>(self, clip: &'a Shape<U>) -> Clipper<'a, Sub, Shape<U>, Op, Tol> {
        Clipper {
            operator: PhantomData,
            tolerance: self.tolerance,
            subject: self.subject,
...
impl<'a, Sub, Clip, Op> Clipper<'a, Sub, Clip, Op, Unknown> {
    pub(crate) fn with_tolerance<Tol>(self, tolerance: Tol) -> Clipper<'a, Sub, Clip, Op, Tol> {
        Clipper {
            operator: PhantomData,
            subject: self.subject,
            clip: self.clip,

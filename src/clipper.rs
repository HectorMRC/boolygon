...
use crate::{
    ...
    Edge, Geometry, IsClose, MaybePair, Shape, Vertex,
};
    ...
    Op: Operator<T>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let sibling_intersection = sibling.intersection.as_ref()?;
        if !sibling_intersection.kind.is_vertex() {
            self.direction = Op::direction(self.clipper.into(), &sibling);
            self.next = Some(self.direction.next(&sibling));
            return Some(node)
        }

        if !intersection.kind.is_vertex() {
            self.next = Some(self.direction.next(&node));
            return Some(node);
        }
        ...
        //     .as_ref()
        //     .and_then(|next| next.intersection.as_ref())
        //     .map(|intersection| intersection.sibling)
        //     .or(Some(candidate));

        // if let Some(intersection) = &node.intersection
        //     && intersection.kind.is_vertex()
        //     && let Some(sibling) = &self.graph.nodes[intersection.sibling]
        //     && let Some(sibling_intersection) = &sibling.intersection 
        // {
        //     if sibling_intersection.kind.is_vertex() {
        //         // TODO
        //     } else {
        //         self.direction = Op::direction(self.clipper.into(), &sibling);
        //         let candidate = self.direction.next(&sibling);
        //         self.next = self.graph.nodes[candidate]
        //             .as_ref()
        //             .and_then(|next| next.intersection.as_ref())
        //             .map(|intersection| intersection.sibling)
